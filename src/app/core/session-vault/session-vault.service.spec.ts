...
describe('SessionVaultService', () => {
  ...
  describe('set', () => {
    it('saves the session in preferences', async () => {
      ...
      expect(Preferences.set).toHaveBeenCalledWith({ key: 'auth-session', value: JSON.stringify(session) });
    });
  });
  ...
  describe('get', () => {
    it('gets the session from preferences', async () => {
      ...
      expect(Preferences.get).toHaveBeenCalledWith({ key: 'auth-session' });
    });
  ...
  describe('clear', () => {
    it('clears the preferences', async () => {
      ...
      expect(Preferences.remove).toHaveBeenCalledWith({ key: 'auth-session' });
    });
  });
});
